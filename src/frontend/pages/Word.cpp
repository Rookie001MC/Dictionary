...
void WordPage::draw()
{
    ...
    if (CheckCollisionPointRec(mousePos, rec_result[i]) && mousePos.y > 180 && !dropDownBox)
        DrawRectangleGradientV(rec_result[i].x, rec_result[i].y, rec_result[i].width, rec_result[i].height, GetColor(RESULT_COLOR_CONTAINER_HOVER), GetColor(RESULT_COLOR_CONTAINER_HOVER));
    ...
    for (int j = 0; j < std::min(2, int(words[i].getDefinitionCount())); j++) {
        ...
        if (definitionPosition.x + textSize.x <= rec_result[i].x + rec_result[i].width &&
            ...
            DrawTextEx(Resources::wordFontBold, words[i].getDefinition(j).c_str(), definitionPosition, 25, 1, WHITE);
        }
    }
}
